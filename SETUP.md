...
## Necessary Packages
The "Linear regression, classification, and resampling" module uses its own isolated environment called `project-env` so that packages don’t conflict with other projects. 
...
1. Create a virtual environment called `project-env`:
    ```
    uv venv project-env --python 3.11
    ```
